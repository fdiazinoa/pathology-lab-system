...
import {
    ...
    Mic, MicOff, Wand2, Package, ArrowLeft, History,
    DollarSign, Shield, FileKey, Share2, MessageSquare, CheckSquare, Eye
} from 'lucide-react';
...
import { runAIAnalysis } from '../ai/aiOrchestrator';
import { checkReportConsistency } from '../services/consistencyService';
import ConsistencyAlerts from '../components/ConsistencyAlerts';

const CaseManager = () => {
    const navigate = useNavigate();
    const { id } = useParams();
    const { patients, addCase, updateCase, deleteCase, getCase, cases, insurers, organs, doctors, settings, updateCaseInterconsultation, exams, currentUser, roles, isProductionMode, logUsageEvent } = useData();
    ...
    const [showSecondLookModal, setShowSecondLookModal] = useState(false);
    const [secondLookReason, setSecondLookReason] = useState('');
    ...
    const toggleSecondLook = async () => {
        const isCurrentlyActive = formData.secondLook?.active;

        if (isCurrentlyActive) {
            // Deactivate
            const updatedSecondLook = {
                ...formData.secondLook,
                active: false,
                unmarkedAt: new Date().toISOString(),
                unmarkedBy: currentUser?.name
            };

            setFormData(prev => ({ ...prev, secondLook: updatedSecondLook }));

            if (id && currentCase) {
                updateCase({
                    ...currentCase,
                    secondLook: updatedSecondLook,
                    auditLogs: [...(currentCase.auditLogs || []), {
                        date: new Date().toISOString(),
                        user: currentUser?.name || 'Sistema',
                        action: 'Segundo Vistazo Desactivado',
                        details: 'El caso ya no requiere revisión adicional.'
                    }]
                });
                logUsageEvent('SecondLook', 'Deactivated', { caseId: id });
            }
        } else {
            // Activate - Show modal for reason
            setShowSecondLookModal(true);
        }
    };

    const confirmSecondLook = () => {
        const updatedSecondLook = {
            active: true,
            reason: secondLookReason,
        ...
        if (id && currentCase) {
            updateCase({
                ...
                auditLogs: [...(currentCase.auditLogs || []), {
                    date: new Date().toISOString(),
                    user: currentUser?.name || 'Sistema',
                    action: 'Segundo Vistazo Activado',
                    details: `Motivo: ${secondLookReason || 'No especificado'}`
                }]
            });
            logUsageEvent('SecondLook', 'Activated', { caseId: id, reason: secondLookReason });
        }

        setShowSecondLookModal(false);
        setSecondLookReason('');
    };
    ...
    const [formData, setFormData] = useState({
        ...
        secondLook: { active: false, reason: '' }
    });
    ...
    const [preservedNames, setPreservedNames] = useState({ patientName: '', doctorName: '' });

    // Effect to load case data when editing
    useEffect(() => {
        if (id && cases.length > 0) {
            const existingCase = cases.find(c => c.id === id);
            if (existingCase) {
                console.log("Editing Case:", existingCase);

                // Fallback for Exam ID if missing but Type exists
                let resolvedExamId = existingCase.examId || '';
                if (!resolvedExamId && existingCase.type && exams.length > 0) {
                    const foundExam = exams.find(e => e.name === existingCase.type);
                    if (foundExam) resolvedExamId = foundExam.id;
                }

                // Fallback for Organ if exact match fails (trim)
                let resolvedOrgan = existingCase.organ || '';

                // Preserve names for fallback display
                setPreservedNames({
                ...
                setFormData({
                    ...
                    secondLook: existingCase.secondLook || { active: false, reason: '' }
                });
            }
        }
    }, [id, cases, exams]);
    ...
    const handleAnalyze = async () => {
        ...
        try {
            const orchestratorInput = {
                ...
                textData: formData.microscopy,
                apiKey: settings.openaiApiKey,
                options: { stopOnLowQuality: true }
            };

            const fullResults = await runAIAnalysis(orchestratorInput);

            // For now, we keep the existing aiResults structure for compatibility with AIAnalysisPanel
            // which expects { suggestions: [], similarCases: [] }
            // We map the orchestrator's 'differentials' result to this structure.
            if (fullResults.differentials) {
                setAiResults({
                    suggestions: fullResults.differentials.differential_diagnoses.map(d => ({
                        diagnosis: d.name,
                        probability: d.confidence.charAt(0).toUpperCase() + d.confidence.slice(1),
                        reasoning: "Sugerencia basada en análisis histopatológico.",
                        type: d.name.toLowerCase().includes('carcinoma') || d.name.toLowerCase().includes('maligno') ? 'Maligno' : 'Benigno'
                    })),
                    similarCases: fullResults.differentials.similarCases.map((sc, idx) => ({
                        id: `SIM-${idx}`,
                        diagnosis: sc.diagnosis,
                        description: sc.description,
                        imageUrl: sc.imageUrl || 'https://via.placeholder.com/150'
                    }))
                });
            } else {
                setAiResults(null);
            ...
            if (fullResults.triage) {
                setFormData(prev => ({ ...prev, aiClassification: fullResults.triage }));
            }
            if (fullResults.qc) {
                setFormData(prev => ({ ...prev, qualityControl: fullResults.qc }));
            }
            if (fullResults.metrics) {
                setFormData(prev => ({
                    ...
                    quantitativeResults: {
                        ...
                        mitosis: fullResults.metrics
                    }
                }));
            }

        } catch (error) {
            console.error("Error analyzing with orchestrator:", error);
    ...
    const handleSave = async (e) => {
        ...
        // Run consistency check before saving
        const consistency = checkReportConsistency(formData);
    ...
    const handleDelete = async () => {
        ...
        if (isProductionMode) {
            const confirmation = window.prompt("MODO PRODUCCIÓN: Esta acción es destructiva e irreversible. Para confirmar la eliminación, escriba 'BORRAR':");
            if (confirmation !== 'BORRAR') {
                alert("Eliminación cancelada. El código de confirmación no coincide.");
                return;
            }
        } else {
            if (!window.confirm("¿Está seguro de eliminar este caso?")) return;
        }
    ...
    return (
        <div className="flex gap-6 max-w-[1600px] mx-auto pb-20 px-4">
            <div className={`flex-1 space-y-6 transition-all duration-300 ${showComparator ? 'max-w-[calc(100%-400px)]' : 'max-w-5xl mx-auto'}`}>
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center gap-2 bg-white p-1 rounded-lg border border-border shadow-sm">
                        <Button
                            variant={showComparator ? "primary" : "ghost"}
                            size="sm"
                            onClick={() => setShowComparator(!showComparator)}
                            className={!showComparator ? "text-text-secondary hover:text-primary" : ""}
                        >
                            <History size={18} className={showComparator ? "mr-2" : "mr-1"} />
                            <span className="text-sm">{showComparator ? 'Cerrar Comparador' : 'Comparar'}</span>
                        </Button>

                        <div className="w-px h-6 bg-border mx-1" />

                        <Button
                            variant={isConversationalMode ? "primary" : "ghost"}
                            size="sm"
                            onClick={() => setIsConversationalMode(!isConversationalMode)}
                            className={!isConversationalMode ? "text-text-secondary hover:text-blue-600" : "bg-blue-600 hover:bg-blue-700"}
                        >
                            <Wand2 size={18} className={isConversationalMode ? "mr-2" : "mr-1"} />
                            <span className="text-sm">{isConversationalMode ? 'Cerrar Asistente' : 'Asistente IA'}</span>
                        </Button>

                        <div className="w-px h-6 bg-border mx-1" />

                        <Button
                            variant={formData.secondLook?.active ? "primary" : "ghost"}
                            size="sm"
                            onClick={toggleSecondLook}
                            className={formData.secondLook?.active ? "bg-indigo-600 hover:bg-indigo-700" : "text-text-secondary hover:text-indigo-600"}
                        >
                            <Eye size={18} className={formData.secondLook?.active ? "mr-2" : "mr-1"} />
                            <span className="text-sm">{formData.secondLook?.active ? 'Quitar Marca' : 'Segundo Vistazo'}</span>
                        </Button>
                    </div>
                </div>

                {/* Second Look Notice */}
                {formData.secondLook?.active && (
                    <div className="bg-indigo-50 border border-indigo-200 p-4 rounded-xl flex items-center justify-between shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h4 className="font-bold text-indigo-900">Este caso está marcado para Segundo Vistazo</h4>
                                <p className="text-sm text-indigo-700">
                                    {formData.secondLook.reason ? `Motivo: ${formData.secondLook.reason}` : 'Se requiere una revisión adicional antes de finalizar.'}
                                </p>
                            </div>
                        </div>
                        {formData.secondLook.markedBy && (
                            <div className="text-xs text-indigo-500 text-right">
                                Marcado por {formData.secondLook.markedBy} el {new Date(formData.secondLook.markedAt).toLocaleDateString()}
                            </div>
                        )}
                    </div>
                )}
                ...
                <form onSubmit={handleSave} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="flex flex-col gap-1.5">
                                ...
                                >
                                    ...
                                    {/* Fallback for preserved patient if not in list */}
                                    {formData.patientId && !patients.find(p => p.id === formData.patientId) && (
                                        <option value={formData.patientId}>
                                            {preservedNames.patientName || formData.patientId} (Preservado)
                                        </option>
                                    )}
                                </select>
                            </div>
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                >
                                    ...
                                    {/* Fallback for preserved doctor if not in list */}
                                    {formData.doctorId && !doctors.find(d => d.id === formData.doctorId) && (
                                        <option value={formData.doctorId}>
                                            {preservedNames.doctorName || formData.doctorId} (Preservado)
                                        </option>
                                    )}
                                </select>
                            </div>
                    ...
                    <ConsistencyAlerts
                        ...
                        onRecheck={() => setConsistencyResults(checkReportConsistency(formData))}
                    />
            ...
            {/* Case Comparator Sidebar */}
            {showComparator && (
                <div className="fixed top-0 right-0 h-screen w-[450px] z-40 shadow-2xl">
                    <CaseComparator
                        currentCase={{ id, ...formData }}
                        allCases={cases}
            ...
            <Modal
                isOpen={showSecondLookModal}
                onClose={() => setShowSecondLookModal(false)}
                onConfirm={confirmSecondLook}
                title="Activar Segundo Vistazo"
                confirmText="Activar"
                cancelText="Cancelar"
            >
                <div className="space-y-4">
                    <p className="text-sm text-text-secondary">
                        Indique el motivo por el cual este caso requiere una revisión adicional (opcional).
                    </p>
                    <Input
                        placeholder="Ej: Caso complejo, revisar diagnóstico..."
                        value={secondLookReason}
                        onChange={(e) => setSecondLookReason(e.target.value)}
                        autoFocus
                    />
                </div>
            </Modal>
